...
const UsersTable = ({isDarkMode}) => {
	...
	const data = [
		{id: 1,name: 'test', type: 'admin', scope: 'all'}
	]
		...
		<div className="relative">
			...
			>
				<div className='flex justify-between items-center mb-6'>
					<h2 className={`${isDarkMode ? 'text-xl font-semibold text-gray-100' : 'text-xl font-semibold text-black'}`}>User</h2>
					<div className='relative'>
						<input
							type='text'
						...
						<tbody className='divide-y divide-gray-700'>
							{ data[0] ? (
								...
								>
									...
									<td className='px-6 py-4 whitespace-nowrap'>
										<div className={`${isDarkMode ? 'text-sm text-gray-300': 'text-sm text-black'}`}>{user.type}</div>
									</td>
									<td className='px-6 py-4 whitespace-nowrap'>
										<div className={`${isDarkMode ? 'text-sm text-gray-300': 'text-sm text-black'}`}>{user.scope}</div>
									</td>
			...
			{addSection && (
					<FormUser  isDarkMode={isDarkMode} />
			)}
			<ToastContainer
